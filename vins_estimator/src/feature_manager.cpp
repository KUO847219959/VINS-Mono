...
{
    // 一帧图像中有 很多个特征点
    // image -> int: 相机id, Matrix: 图像中每个特征点的属性
    ...
    for (auto &id_pts : image)
    {
        // 用特征点信息构造一个对象
        // params:
        //      id_pts.second[0].second -> 图像中每个特征点的属性,包括xyz_uv_VxVy
        //      td：???
    ...
    for (auto &it_per_id : feature)
    {
        ...
        // 条件1：
        //    it_per_id.start_frame <= frame_count - 2 : 特征点至少相隔两帧图像
        //    开始帧 + 有跟踪到的该点的其他帧 >= 滑动窗口的倒数第二帧
        //    计算平均像素差
        //    像素差大于某个阈值则认为是 关键帧
...
vector<pair<Vector3d, Vector3d>> FeatureManager::getCorresponding(int frame_count_l, int frame_count_r)
{
    ...
    for (auto &it : feature)
    {
        ...
        if (it.start_frame <= frame_count_l && it.endFrame() >= frame_count_r)
        {
            ...
            int idx_l = frame_count_l - it.start_frame; // frame_count_l-第一次观测到特征点的帧数
