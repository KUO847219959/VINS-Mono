...
void Estimator::processImage(const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &image, const std_msgs::Header &header)
{
    ...
    // tmp_pre_integration是之前IMU 预积分计算的, 多拷贝了一份
    ...
    if(ESTIMATE_EXTRINSIC == 2)
    {
        ...
        if (frame_count != 0)
        {
            ...
            Matrix3d calib_ric;
            // 标定从camera到IMU之间的旋转矩阵 calib_ric
    ...
    if (solver_flag == INITIAL)
    {
        ...
        else   // 没有足够的帧数，图像帧数量+1
            frame_count++;
    }
    else  //  初始化成功solver_flag = NON_LINEAR， 紧耦合的非线性优化
    {
        TicToc t_solve;
        ...
        TicToc t_margin;
        slideWindow();  // 滑窗
        f_manager.removeFailures();  // 移除失败的, 一些不能被三角化的点
...
bool Estimator::initialStructure()
{
    ...
    // SFMFeature 屬性：三角化状态、特征点索引、observation 平面观测、position 位置坐标、深度
    ...
    for (auto &it_per_id : f_manager.feature)
    {
        ...
        tmp_feature.state = false; // 三角化状态state
    ...
    Vector3d relative_T;
    int l;
    // relativePose()返回滑动窗口中第一个满足视差的帧，为l帧，以及RT,可以三角化。
...
bool Estimator::relativePose(Matrix3d &relative_R, Vector3d &relative_T, int &l)
{
    ...
    for (int i = 0; i < WINDOW_SIZE; i++)
    {
        ...
        // getCorresponding() // 返回相机坐标系下的坐标对 z=1
